...
void main(void)
{
    ...
    float face_skin_roi = step(1.99, ai_mask * 255.0) * step(ai_mask * 255.0, 2.01);
    float body_skin_roi = step(3.99, ai_mask * 255.0) * step(ai_mask * 255.0, 4.01);
    float roi_wight = max(face_skin_roi * face_skin, body_skin_roi);
    if ((face_skin_roi + body_skin_roi) > 0.0) {
        vec3 rgb = center_color;//YUV2RGB(center_color);
